...
namespace robot_controller
{
  ...
  private:
    ...
    std::unique_ptr<dynamic_reconfigure::Server<robot_controller::robot_controller_paramConfig>> dynamic_server_compliance_param_;
    ros::NodeHandle dynamic_reconfigure_compliance_param_node_;
    void controlParamCallback(robot_controller::robot_controller_paramConfig &config, uint32_t level);

    // 发布数据
    ros::Publisher paramForDebug;
    robot_controller::paramForDebug param_debug;
