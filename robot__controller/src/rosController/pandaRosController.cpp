...
namespace robot_controller
{
  bool PandaController::init(hardware_interface::RobotHW *robot_hw, ros::NodeHandle &node_handle)
  {
    ...
    std::cout << "[robotController] init1:robot_controller" << std::endl;
    std::cout << "[robotController] init2:robot_controller" << std::endl;

    std::string urdfPath;
    std::string TcpName;
    if (!node_handle.getParam("/panda_withoutHand_dyn", urdfPath))
    {
      ROS_ERROR_STREAM("robot_controller: Could not read parameter urdfPath");
    }
    if (!node_handle.getParam("/TCP_name", TcpName))
    {
      ROS_ERROR_STREAM("robot_controller: Could not read parameter TcpName");
    }
    ...
    {
      ROS_ERROR_STREAM("robot_controller: Could not read parameter arm_id");
      return false;
    }
    std::vector<std::string> joint_names;
    if (!node_handle.getParam("joint_names", joint_names) || joint_names.size() != 7)
    {
      ROS_ERROR("robot_controller: Invalid or no joint_names parameters provided, "
    ...
    {
      ROS_ERROR_STREAM("robot_controller: Error getting model interface from hardware");
      return false;
    }
    try
    {
    ...
    {
      ROS_ERROR_STREAM("robot_controller: Exception getting model handle from interface: " << ex.what());
      return false;
    }
    ...
    {
      ROS_ERROR_STREAM("robot_controller: Error getting state interface from hardware");
      return false;
    }
    try
    {
    ...
    {
      ROS_ERROR_STREAM("robot_controller: Exception getting state handle from interface: " << ex.what());
      return false;
    }
    ...
    {
      ROS_ERROR_STREAM("robot_controller: Error getting effort joint interface from hardware");
      return false;
    }
    for (size_t i = 0; i < 7; ++i)
    {
      ...
      {
        ROS_ERROR_STREAM("robot_controller: Exception getting joint handles: " << ex.what());
        return false;
      }
    }
    ...
    dynamic_server_compliance_param_ = std::make_unique<dynamic_reconfigure::Server<robot_controller::robot_controller_paramConfig>>(dynamic_reconfigure_compliance_param_node_);
    dynamic_server_compliance_param_->setCallback(boost::bind(&PandaController::controlParamCallback, this, _1, _2));

    // 发布者对象
    paramForDebug = node_handle.advertise<robot_controller::paramForDebug>("paramForDebug", 20);
  ...
  {
    std::cout << "[robotController] start1:robot_controller\n";
    std::cout << "[robotController] start2:robot_controller\n";
  ...
  void PandaController::controlParamCallback(robot_controller::robot_controller_paramConfig &config, uint32_t /*level*/)
  {
...
PLUGINLIB_EXPORT_CLASS(robot_controller::PandaController,
